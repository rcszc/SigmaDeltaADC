...
namespace PSAG_LOGGER_PROCESS {
	...
	bool StartLogProcessing(const char* folder) {
		...
		if (!filesystem::exists(folder) || !filesystem::is_directory(folder)) {
			PSAG_LOGGER::PushLogger(LogWarning, PSAG_LOGGER_LABEL, 
				"invalid folder: %s", folder);
			// create system folder.
			if (filesystem::create_directories(folder)) {
				PSAG_LOGGER::PushLogger(LogError, PSAG_LOGGER_LABEL, 
					"create logs folder failed create.");
				return false;
			}
		...
		try {
			...
			PSAG_LOGGER::PushLogger(LogInfo, PSAG_LOGGER_LABEL, "logger thread start thread.");
		}
		catch (const exception& err) {
			PSAG_LOGGER::PushLogger(LogError, PSAG_LOGGER_LABEL, 
				"logger thread start thread, error: %s", err.what());
			return false;
		}
		return true;
	}
	...
	bool FreeLogProcessing() {
		PSAG_LOGGER::PushLogger(LogInfo, PSAG_LOGGER_LABEL, "logger thread close...");
		// 处理日志缓存残留. 2025_02_06 RCSZ.
		this_thread::sleep_for(chrono::milliseconds(500));
		try {
			// set flag(exit) => join thread.
			LogProcessFlag = false;
			if (LogProcessThread->joinable())
				LogProcessThread->join();
		}
		catch (const exception& err) {
			cout << "system close thread, error: " << err.what() << endl;
			return false;
		}
